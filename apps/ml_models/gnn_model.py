...
class FraudGNN(nn.Module):
    ...
    def forward(self, data: Data) -> Dict[str, torch.Tensor]:
        ...
        combined_repr = torch.cat([
            graph_repr,
            torch.mean(node_repr, dim=0, keepdim=True) if batch is None else global_mean_pool(node_repr, batch)
        ], dim=1)
...
class GraphBuilder:
    ...
    def _should_connect(
        ...
        if time_diff <= time_window:
            return (
                txn1['user_id'] == txn2['user_id'] or
                txn1['merchant_id'] == txn2['merchant_id'] or
                txn1.get('ip_address') == txn2.get('ip_address') or
                txn1.get('device_fingerprint') == txn2.get('device_fingerprint')
            )
    ...
    def predict(
        ...
        try:
            ...
            if return_embeddings:
                results['node_embeddings'] = outputs['node_embeddings'].cpu().numpy().tolist()
                results['graph_embeddings'] = outputs['graph_embeddings'].cpu().numpy().tolist()
