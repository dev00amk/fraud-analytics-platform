...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x: torch.Tensor, timestamps: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if timestamps is not None:
            ...
            hour_emb = self.hour_embedding(dt_features['hour'])  # [batch_size, seq_len, d_model//4]
            day_emb = self.day_embedding(dt_features['day'])     # [batch_size, seq_len, d_model//4]
            
            # Combine temporal embeddings
            temporal_emb = torch.cat([hour_emb, day_emb], dim=-1)  # [batch_size, seq_len, d_model//2]
            temporal_emb = self.temporal_proj(temporal_emb)        # [batch_size, seq_len, d_model]
            temporal_emb = temporal_emb.transpose(0, 1)           # [seq_len, batch_size, d_model]
    ...
    def _extract_datetime_features(self, timestamps: torch.Tensor) -> Dict[str, torch.Tensor]:
...
class SequenceBuilder:
    ...
    def build_sequences(
        ...
        recent_txns = [
            ...
            if datetime.fromisoformat(txn['timestamp'].replace('Z', '+00:00')) >= cutoff_time
            and datetime.fromisoformat(txn['timestamp'].replace('Z', '+00:00')) < target_time
        ]
        ...
        for txn in recent_txns[-self.max_seq_len:]:  # Keep only recent transactions
            ...
            timestamps.append(
                datetime.fromisoformat(txn['timestamp'].replace('Z', '+00:00')).timestamp()
            )
