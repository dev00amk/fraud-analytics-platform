...
class EnsembleModel:
    ...
    def __init__(
        ...
        self.transformer_engine = TransformerInferenceEngine(transformer_path, device) if transformer_path else None
    ...
    def predict(
        ...
        if self.transformer_engine and user_transactions:
            transformer_pred = self.transformer_engine.predict(transaction, user_transactions)
    ...
    def _predict_lstm(
        ...
        try:
            ...
            # Convert to tensor
            sequence_tensor = torch.tensor([sequence_data], dtype=torch.float32).to(self.device)
    ...
    def _combine_predictions(self, model_predictions: Dict[str, Dict]) -> Dict[str, float]:
    ...
    def _calculate_dynamic_weights(
        ...
        for model_name in available_models:
            ...
            performance = self.model_performance.get(model_name, {'accuracy': 0.8, 'confidence': 0.8})
            performance_factor = (performance['accuracy'] + performance['confidence']) / 2
    ...
    def update_model_performance(
        ...
        if model_name in self.model_performance:
            ...
            self.model_performance[model_name]['accuracy'] = (
                alpha * accuracy + (1 - alpha) * self.model_performance[model_name]['accuracy']
            )
            self.model_performance[model_name]['confidence'] = (
                alpha * confidence + (1 - alpha) * self.model_performance[model_name]['confidence']
            )
...
class EnsembleInferenceEngine:
    ...
    def get_performance_metrics(self) -> Dict[str, float]:
        ...
        return {
            ...
            'predictions_per_second': 1000 / max(self.performance_metrics['avg_inference_time'], 1)
        }