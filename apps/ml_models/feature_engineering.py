...
class FeatureSet:
    ...
    def to_dict(self) -> Dict[str, float]:
        ...
        for feature_type in [
            ...
            'device_features', 'behavioral_features', 'velocity_features',
    ...
    def get_velocity_features(
        ...
        for window_name, window_seconds in self.time_windows.items():
            ...
            features.update({
                ...
                f'amount_sum_{window_name}': sum(txn['amount'] for txn in window_txns),
                f'amount_avg_{window_name}': np.mean([txn['amount'] for txn in window_txns]) if window_txns else 0.0,
                f'amount_std_{window_name}': np.std([txn['amount'] for txn in window_txns]) if len(window_txns) > 1 else 0.0,
                f'amount_max_{window_name}': max([txn['amount'] for txn in window_txns]) if window_txns else 0.0,
                f'amount_min_{window_name}': min([txn['amount'] for txn in window_txns]) if window_txns else 0.0,
            })
        
        # Calculate velocity ratios
        features.update({
            'velocity_ratio_1m_1h': features['count_1m'] / max(features['count_1h'], 1),
            'velocity_ratio_5m_1h': features['count_5m'] / max(features['count_1h'], 1),
            'velocity_ratio_1h_24h': features['count_1h'] / max(features['count_24h'], 1),
            'amount_ratio_1m_1h': features['amount_sum_1m'] / max(features['amount_sum_1h'], 1),
            'amount_ratio_1h_24h': features['amount_sum_1h'] / max(features['amount_sum_24h'], 1),
        })
        
        return features
    
    def _get_transactions(self, entity_key: str, current_timestamp: float) -> List[Dict]:
        """Get transactions from cache or Redis."""
        transactions = []
...
class BehavioralAnalyzer:
        ...
        if self.redis_client:
            try:
                redis_profile = {
                    ...
                    'avg_amount': np.mean(profile['amounts']) if profile['amounts'] else 0.0,
                    'std_amount': np.std(profile['amounts']) if len(profile['amounts']) > 1 else 0.0,
    ...
    def get_behavioral_features(
        ...
        features['merchant_familiarity'] = merchant_count / max(len(profile['transactions']), 1)
        features['merchant_diversity'] = total_merchants / max(len(profile['transactions']), 1)
        
        # Payment method familiarity
        payment_count = profile['payment_methods'].get(current_payment, 0)
        features['payment_method_familiarity'] = payment_count / max(len(profile['transactions']), 1)
        
        # Device familiarity
        device_count = profile['devices'].get(current_device, 0)
        features['device_familiarity'] = device_count / max(len(profile['transactions']), 1)
        features['device_diversity'] = len(profile['devices']) / max(len(profile['transactions']), 1)
        
        # IP familiarity
        ip_count = profile['ips'].get(current_ip, 0)
        features['ip_familiarity'] = ip_count / max(len(profile['transactions']), 1)
        features['ip_diversity'] = len(profile['ips']) / max(len(profile['transactions']), 1)
        
        # Amount deviation
        if profile['amounts']:
            avg_amount = np.mean(profile['amounts'])
            std_amount = np.std(profile['amounts']) if len(profile['amounts']) > 1 else avg_amount * 0.1
            features['amount_deviation'] = abs(current_amount - avg_amount) / max(std_amount, 1)
            features['amount_percentile'] = np.percentile(profile['amounts'], 
                                                        [25, 50, 75, 90, 95, 99])
            features['amount_z_score'] = (current_amount - avg_amount) / max(std_amount, 1)
        else:
        ...
        if profile['timestamps']:
            ...
            features['hour_familiarity'] = hour_counts[current_hour] / max(len(hours), 1)
            
            # Day of week pattern
            days = [datetime.fromtimestamp(ts).weekday() for ts in profile['timestamps']]
            ...
            features['time_since_last'] = (current_time - last_timestamp) / 3600  # Hours
        else:
...
class DeviceFingerprinter:
            ...
            try:
                ...
                if device_data:
                    return {
                        ...
                        'device_transaction_count': float(device_data.get(b'txn_count', 0)),
    ...
    def _analyze_user_agent(self, user_agent: str) -> Dict[str, float]:
        ...
        features['is_mobile'] = float(any(indicator in ua_lower for indicator in mobile_indicators))
        
        # Bot detection
        bot_indicators = ['bot', 'crawler', 'spider', 'scraper', 'curl', 'wget']
        features['is_bot'] = float(any(indicator in ua_lower for indicator in bot_indicators))
        
        # Suspicious patterns
        features['has_suspicious_ua'] = float(
            len(user_agent) < 10 or 
            len(user_agent) > 500 or
            user_agent.count('(') != user_agent.count(')') or
            'null' in ua_lower or
            'undefined' in ua_lower
        )
...
class FeatureEngineeringPipeline:
    ...
    def extract_features(
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            futures = {
                'transaction': executor.submit(self._extract_transaction_features, transaction),
                'velocity': executor.submit(self._extract_velocity_features, transaction),
                'behavioral': executor.submit(self._extract_behavioral_features, transaction, user_transactions),
                'device': executor.submit(self._extract_device_features, transaction),
                'temporal': executor.submit(self._extract_temporal_features, transaction),
    ...
    def _extract_velocity_features(self, transaction: Dict[str, Any]) -> Dict[str, float]:
        ...
        for entity_type, entity_id in entities:
            if entity_id:
                ...
                self.velocity_tracker.update_velocity(entity_type, entity_id, transaction)
                
                # Get velocity features
                velocity_features = self.velocity_tracker.get_velocity_features(entity_type, entity_id)
    ...
    def _extract_network_features(self, transaction: Dict[str, Any]) -> Dict[str, float]:
    ...
    def _add_derived_features(self, feature_set: FeatureSet, transaction: Dict[str, Any]) -> FeatureSet:
        ...
        merchant_avg_amount = all_features.get('velocity_features_merchant_amount_avg_24h', 1.0)
        ...
        overall_familiarity = all_features.get('behavioral_features_overall_familiarity', 0.5)
    ...
    def _update_metrics(self, extraction_time: float):
        ...
        self.metrics['avg_extraction_time'] = (
            alpha * extraction_time +
            (1 - alpha) * self.metrics['avg_extraction_time']
        )
    ...
    def get_performance_metrics(self) -> Dict[str, float]:
        ...
        return {
            ...
            'extractions_per_second': 1000 / max(self.metrics['avg_extraction_time'], 1)
        }
