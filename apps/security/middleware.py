...
class IdempotencyMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request: HttpRequest) -> Optional[HttpResponse]:
        ...
        if cached_response:
            logger.info(f"Returning cached response for duplicate request: {fingerprint}")
            return JsonResponse(
    ...
    def _should_check_idempotency(self, request: HttpRequest) -> bool:
        ...
        return (
            ...
            any(path in request.path for path in self.idempotent_paths)
        )
...
class RequestValidationMiddleware(MiddlewareMixin):
    ...
    def process_request(self, request: HttpRequest) -> Optional[HttpResponse]:
        ...
        if self._contains_malicious_content(request):
            logger.warning(f"Malicious content detected in request from {request.META.get('REMOTE_ADDR')}")
            return JsonResponse({
