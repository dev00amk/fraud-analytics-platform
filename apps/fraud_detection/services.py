...
class AdvancedFraudDetectionService:
    ...
    async def analyze_transaction_async(
        ...
        try:
            ...
            feature_time = (
                datetime.now() - feature_extraction_start
            ).total_seconds() * 1000
            ...
            rule_results = await self._apply_rules_async(
                transaction_data, user, features
            )
            ...
            final_results.update(
                {
                    "performance_metrics": {
                        ...
                        "features_extracted": (
                            len(features.to_dict()) if features else 0
                        ),
                    },
    ...
    async def _extract_features_async(
        self, transaction_data: Dict[str, Any], context: Optional[Dict[str, Any]] = None
    ):
        ...
        try:
            ...
            merchant_transactions = (
                context.get("merchant_transactions", []) if context else []
            )
    ...
    async def _apply_rules_async(
        self, transaction_data: Dict[str, Any], user: Any, features
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for rule in rules:
                result = await self._evaluate_rule_async(
                    rule, transaction_data, features
                )
                results.append(
                    {
                        "rule_id": str(rule.id),
    ...
    async def _evaluate_rule_async(
        self, rule: FraudRule, transaction_data: Dict[str, Any], features
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            required_conditions = conditions.get("_meta", {}).get(
    ...
    def _evaluate_condition(
        ...
        if condition_type == "threshold":
            ...
            return {"met": False, "value": value, "threshold": None, "confidence": 0.0}
    ...
    def _get_feature_summary(self, features) -> Dict[str, Any]:
        ...
        top_risk_features = dict(
            sorted(risk_features.items(), key=lambda x: x[1], reverse=True)[:10]
        )

        return {
            "total_features": len(feature_dict),
            "top_risk_features": top_risk_features,
            "velocity_features_count": len(
                [k for k in feature_dict.keys() if "velocity" in k]
            ),
            "behavioral_features_count": len(
                [k for k in feature_dict.keys() if "behavioral" in k]
            ),
            "device_features_count": len(
                [k for k in feature_dict.keys() if "device" in k]
            ),
        }
    ...
    def _generate_explanation(
        ...
        if len(triggered_rules) > 2:
            explanation["key_factors"].append(
                f"Multiple fraud rules triggered ({len(triggered_rules)})"
            )
    ...
    def _get_fallback_result(
        self, transaction_data: Dict[str, Any], error: str
    ) -> Dict[str, Any]:
    ...
    def update_model_performance(
        ...
        if self.ensemble_engine and hasattr(
